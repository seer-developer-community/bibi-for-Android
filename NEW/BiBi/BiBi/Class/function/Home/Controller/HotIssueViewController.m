...
@interface HotIssueViewController ()<UITableViewDataSource,UITableViewDelegate>
//@property (nonatomic,strong) BitsharesWalletObject *wallet;

@property (nonatomic,assign) BOOL connected;

@property(nonatomic,strong)UITableView *hotlssueTaleView;

@property(nonatomic,strong)NSMutableArray *hotIssueArry;
...
@implementation HotIssueViewController
...
//    [self setupURL];
    ...
    [self.view addSubview:self.hotlssueTaleView];
    //添加没数据
...
-(void)setTopView{
    ...
    self.navView.title = NSLocalizedStringFromTable(@"HOT_Topics",@"Internation", nil);
...
-(void)clicked{
    ...
    [[BitsharesWalletObject BitsharesWalletObjectManager] getPlatform:@"get_houses" unploadParamsTotalParams:@[oneStr,twoStr,thirsStr] paltNum:2 success:^(PlatAccount *result) {
        __weak typeof(self) weakSelf = self;
        //
        weakSelf.hotIssueArry = [NSMutableArray array];
        //        [weakSelf.dataArray addObjectsFromArray:result.rooms];
        ...
        //
        //        });
        //        NSMutableDictionary * tempDic = [NSMutableDictionary dictionaryWithCapacity:100];
        for (id roomID  in result.rooms) {
            if ([roomID isKindOfClass:[NSString class]]) {
                DetailsModel *model = [[DetailsModel alloc] init];
                model.roomId = roomID;
                [weakSelf.hotIssueArry addObject:model];;
            }
        }
        ...
        for (int room = 0; room < result.rooms.count; room++)
        {
            [[BitsharesWalletObject BitsharesWalletObjectManager] getSeerRoom:@"get_seer_room"  fristString:result.rooms[room] nLimint:0 secondInt:100 success:^(RoomAccount *result2) {
                ...
                finisCount++;
                //                result2.identifier
                for (DetailsModel * modle  in weakSelf.hotIssueArry) {
...
//                            }
                ...
                if (finisCount == result.rooms.count) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [weakSelf.hotlssueTaleView reloadData];
                        [self.loadingView cancelView];
...
//初始化tableview
-(UITableView *)hotlssueTaleView{
    
    if (!_hotlssueTaleView) {
        
        _hotlssueTaleView =[[UITableView alloc]initWithFrame:CGRectMake(0, self.navView.bottom, kScreenW, kScreenH-self.navView.height) style:UITableViewStyleGrouped];
        _hotlssueTaleView.delegate = self;
        _hotlssueTaleView.dataSource =self;
        
        _hotlssueTaleView.estimatedRowHeight = 180;
        
        [ _hotlssueTaleView setSeparatorStyle:UITableViewCellSeparatorStyleNone];
        
        _hotlssueTaleView.backgroundColor = UIColorFromHexValue(0xf4f6fd);
        
    }
    
    
    return _hotlssueTaleView;
}
