...
- (AssetAmountObject *)caculateFeeWithFeeDic2:(NSDictionary *)feeDictionary payFeeAsset:(AssetObject *)asset {
    long baseAmount;
    if ([asset.symbol isEqualToString:@"PFC"]){
        baseAmount = [feeDictionary[@"fee"] integerValue];
...
//    if ([asset.symbol isEqualToString:@"PFC"]) {
//        return [[AssetAmountObject alloc] initFromAssetId:asset.identifier amount:(finalResult.longValue)*1000];
//
//    }else{
         return [[AssetAmountObject alloc] initFromAssetId:asset.identifier amount:finalResult.longValue];
//    }
