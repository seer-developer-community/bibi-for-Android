...
+ (instancetype)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        updateManager.appStoreUrl =@"https://www.seer.best/index.php?id=226";
        //
...
+ (void)CheckVersionUpadateWithForce:(BOOL)isForce {
    ...
    [[CZNetWorkTool sharedNetWorkTool] GetobjectWithURLString:@"https://www.seer.best/index.php?id=226" with:nil completeBlock:^(id responseObject) {
        
        if (responseObject[@"result"]) {
            //取出线上的版本号
            NSString *onlineVersion = [responseObject[@"result"] firstObject][@"iosVersion"];
            //获取更新版本信息
            if ([UPDATEMESSEGE isEqualToString:@"新版本"]) {
                 manager.updateMessage = [responseObject[@"result"] firstObject][@"iosReleaseNotesCn"];
            }else{
                 manager.updateMessage = [responseObject[@"result"] firstObject][@"iosReleaseNotesEn"];
            }
            ...
            switch ([manager compareOnlineVersion:onlineVersion toVersion:manager.locVersion]) {
                ...
                case 1:
                {  //是否强制更新
                    if([responseObject[@"result"] firstObject][@"iosIsforce"]){
                        [manager showAlert:YES iosUpdataurl:[responseObject[@"result"] firstObject][@"iosUpdataurl"]];
                    }
                    //带取消按钮更新弹窗
                    else{
                        [manager showAlert:NO iosUpdataurl:[responseObject[@"result"] firstObject][@"iosUpdataurl"]];
                    }
                }
                    break;
                default:
 ...
 */
- (void)showAlert:(BOOL)isforce iosUpdataurl:(NSString*)iosUpdataurl{
    ...
    UIAlertAction *ok = [UIAlertAction actionWithTitle:UPDATEOK style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
         ...
         [[UIApplication sharedApplication] openURL:[NSURL URLWithString:iosUpdataurl]];
        
        //点击后还要重新弹出 始终在app视图上显示
        if (isforce) {
            [self showAlert:YES iosUpdataurl:iosUpdataurl];
        }
    }];
